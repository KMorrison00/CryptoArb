...
'''current 2 exchanges seem to consistently have a 0.05% gap
TO DO:
-implement brokerage account normalization to account for skewed price differences, ie one exchange consistently lags
-long term goals:
    -implement multiple exchange checking, may need ot change strategy or have massed enough wealth to have buffer on
    many many many exchanges
    - parralelize computation of checking multiple exchanges, perhaps check all for price gaps and then sort the list
    then use different api to just order from all? 
    -fully document this whole goddamn file as well as any changes
'''
